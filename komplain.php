...
// }
$pegawai = $konek->query("SELECT * FROM pegawai ")->fetch_array(MYSQLI_BOTH);
            ...
            <li>
                <ul class="collapsible collapsible-accordion">
                  <li><a href="<?=$konfig['url']?>/index.php" class="collapsible-header waves-effect"><i class="fa fa-home"></i>Dashboard</a></li>
                    ...
                    <li><a href="<?=$konfig['url']?>/komplain.php" class="collapsible-header waves-effect active"><i class="fa fa-minus"></i><?php if ($komplain != 0) { echo'<span class="badge badge-warning">'.$komplain.'</span>'; } ?> Komplain</a></li>
        ...
        <!-- /.Navbar -->
          ...
          <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
            ...
            <div class="btn-group mr-2">
              <a href="<?=$konfig['url']?>/komplain.php" class="btn btn-sm btn-danger active">Tabel Data</a>
            </div>
          </div>
          ...
          <div class="table-responsive">
            <table id="tablee" class="table table-striped table-bordered">
            	...
            	<tbody>
            		<?php
                $no = 1;
                $keberatan = $konek->query("SELECT keberatan.id_sub_nilai, max(keberatan.tgl_kirim) as tgl_kirim, max(keberatan.status) as status FROM penilaian, keberatan WHERE penilaian.id_sub_nilai=keberatan.id_sub_nilai AND penilaian.nik='$pegawai[nik]' GROUP BY keberatan.id_sub_nilai");
                while ($row = $keberatan->fetch_array(MYSQLI_BOTH)) {
                  $komplain = $konek->query("SELECT * FROM keberatan WHERE id_sub_nilai='$row[id_sub_nilai]' AND status='2'")->num_rows;
                ?>
            		<tr>
            			<td><?=$no?></td>
            			<td><a href="?page=view&id=<?=$row['id_sub_nilai']?>" style="color:green;"><?=$row['id_sub_nilai']?></a></td>
                  <td><?php if ($row['status'] == 2) { echo'<span class="badge badge-success">'.$komplain.' Pesan Baru</span> '; } ?><?=$row['tgl_kirim']?> (<?=waktu($row['tgl_kirim'])?>)</td>
            		</tr>
            		<?php $no++; }?>
            	</tbody>
            </table>
